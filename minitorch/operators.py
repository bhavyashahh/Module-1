...
def map(fn):
    ...
    def map_fn(list_input):
        outlist = []
        for elt in list_input:
            outlist.append(fn(elt))
        return outlist
    return map_fn
...
def zipWith(fn):
    ...
    def zip_fn(ls1, ls2):
        new_ls = []
        for elt1, elt2 in zip(ls1, ls2):
            new_ls.append(fn(elt1, elt2))
        return new_ls
    return zip_fn
...
def reduce(fn, start):
    ...
    def reduce_fn(ls):
        running_out = start
        for elt in ls:
            running_out = fn(elt, running_out)
        return running_out
    return reduce_fn
...
def sum(ls):
    ...
    reduce_fn = reduce(add, 0)
    return reduce_fn(ls)


def prod(ls):
    "Product of a list using :func:`reduce` and :func:`mul`."
    reduce_fn = reduce(mul, 1)
    return reduce_fn(ls)
